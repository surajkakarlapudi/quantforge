...
class MetricPeriod:
    ...
    meaning is attached (mirrors Phase 4's :class:`~quantforge.canonical.period`
...
class PitMetricValue(_MetricValueBase):
    ...
    history. Computed *only* from :class:`~quantforge.availability.resolve.PitValue`
...
class RevisedMetricValue(_MetricValueBase):
    ...
    def reinterpret_as_pit(self, engine: object, as_of: datetime) -> PitMetricValue:
        ...
        :class:`~quantforge.metrics.engine.MetricEngine`; typed as ``object`` here
