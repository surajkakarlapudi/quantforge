...
Phase 23 needs the standard-normal cumulative distribution function ``Φ`` (for the
Probabilistic and Deflated Sharpe Ratios) and its inverse ``Z⁻¹`` (for the
expected-maximum-Sharpe threshold of a research campaign of ``N`` trials). Neither
exists anywhere in the codebase, and neither may be taken from ``math`` /
``statistics`` / numpy: a ``float`` ``erf`` would break the project's exact-``Decimal``
determinism (Principle 10) - the same input could round differently across platforms or
libc versions, and a sealed campaign id could then drift. So this module computes both
in stdlib :class:`~decimal.Decimal` under an explicit :class:`~decimal.Context`, exactly
the way the risk/optimization layers already lean on ``Decimal.sqrt`` /
``Decimal.exp`` (both correctly rounded per the General Decimal Arithmetic
Specification, hence bit-identical on every platform).
...
#: argument Phase 23 evaluates (``|z|`` well under 10), far inside this backstop; it
...
#: any guarded working precision Phase 23 uses - for every ``p`` the engine evaluates. A
...
#: any precision Phase 23 uses, so the true quantile of every evaluated ``p`` (all with
#: ``|Z⁻¹(p)| < 10``) is strictly bracketed.
...
def standard_normal_ppf(p: Decimal, *, context: Context) -> Decimal:
    ...
    exit). ``p`` outside the open unit interval is a caller bug (the engine only ever
    passes ``1-1/N`` and ``1-1/(N·e)`` with ``N ≥ 2``) and raises rather than being
    guessed.
    """
    if not (_ZERO < p < _ONE):
