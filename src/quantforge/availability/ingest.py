...
* **``retrieved_at`` joined only at derivation (Decision 1).** It is read from the
  Phase 1 :class:`~quantforge.sec.artifacts.AcquisitionMetadata` here and placed
  on :class:`~quantforge.availability.model.FilingEvidence` — never propagated to
...
from quantforge.availability.policy import derive
