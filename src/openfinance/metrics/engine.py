...
1. looks a formula up by ``metric_key`` in the :class:`FormulaRegistry` (fail-closed
   on unknown, §6);
2. obtains a Phase 5 :class:`~openfinance.availability.resolve.PointInTimeResolver`
...
3. hands the filer's facts + resolver + boundary to the **pure**
   :class:`~openfinance.metrics.evaluate.MetricEvaluator`.
...
class MetricEngine:
    ...
    def metric_as_of(
        ...
        :class:`~openfinance.availability.errors.ModeError`, invariant 15). Returns a
