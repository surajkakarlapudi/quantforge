...
Phase 4 :class:`~openfinance.canonical.version.CanonicalFactVersion` pattern (id is
...
* :class:`AvailabilityPolicy` — the **versioned, form-scoped, era-bounded rule**
  that ``derive(evidence, policy)`` (see :mod:`openfinance.availability.policy`)
...
from openfinance.availability.calendar import EASTERN_DST_REGIME_FROM
...
class AvailabilityRule:
    ...
    rule_kind:
        ...
        value fails closed
        (:class:`~openfinance.availability.errors.PolicyConfigurationError`) — we
    ...
    calendar:
        ...
        implemented (see :mod:`openfinance.availability.calendar`).
