...
:class:`~openfinance.availability.model.FilingEvidence` and one selected
:class:`~openfinance.availability.version.AvailabilityPolicy`, it produces a
:class:`~openfinance.availability.model.FilingAvailability` triple
...
* **Deterministic (invariant 13).** No wall-clock, no RNG, no input-order
  ...
  :mod:`openfinance.availability.calendar` (no host tz database).
...
→ :class:`~openfinance.availability.errors.PolicyConfigurationError` (we never
